...
export const PromptInput = ({ value, onChange, placeholder }: PromptInputProps) => {
  ...
  const getPromptTips = () => {
    const tips = [
      "Start with an action verb (e.g., 'Create', 'Analyze', 'Write')",
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        <Textarea
          ...
          placeholder={placeholder || "Example: Create a comprehensive marketing strategy for a sustainable fashion startup targeting Gen Z consumers, including social media campaigns, influencer partnerships, and budget allocation."}
          className="min-h-[140px] resize-none border-gray-300 focus:border-blue-500 focus:ring-blue-500 pr-16"
      ...
      <div id="prompt-help" className="text-xs text-gray-500 space-y-1">
        <p>
          ...
          <span className="ml-2">
            <span className="text-red-500">• &lt;50 chars: Too short</span>
            <span className="ml-3 text-yellow-500">• 50-100 chars: Needs detail</span>
            <span className="ml-3 text-green-500">• 100+ chars: Good length</span>
          </span>
        </p>
        {value.length === 0 && (
